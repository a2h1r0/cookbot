...
export function useRecipes() {
  ...
  const { generate, loading: geminiLoading, error: geminiError } = useGemini();  const searchRecipes = useCallback(
    async (filters: SearchFilters) => {
      const requestId = Math.random().toString(36).substr(2, 9);
      const startTime = Date.now();
      
      console.log(`[RECIPES-${requestId}] === Recipe Search Started ===`);
      console.log(`[RECIPES-${requestId}] Timestamp: ${new Date().toISOString()}`);
      ...
      try {
        ...
        try {
          console.log(`[RECIPES-${requestId}] Parsing recipes from response...`);
          ...
          console.error(`[RECIPES-${requestId}] JSON解析エラー after ${totalDuration}ms:`, {
            parseError,
        ...
        console.error(`[RECIPES-${requestId}] Recipe search error after ${totalDuration}ms:`, {
        ...
        console.log(`[RECIPES-${requestId}] Recipe search completed in ${finalDuration}ms`);
        setLoading(false);
      }
    },
    [generate]
