...
export function useGemini(): UseGeminiReturn {
  ...
  const generateSubstitution = useCallback(
    async (
      ...
      console.log(
        `[CLIENT-${requestId}] === Gemini Substitution Request Started ===`
      );
      console.log(
        `[CLIENT-${requestId}] Timestamp: ${new Date().toISOString()}`
      );
      console.log(`[CLIENT-${requestId}] Substitution Request:`, {
        promptLength: request.prompt?.length || 0,
        promptPreview:
          request.prompt?.substring(0, 100) +
          (request.prompt?.length > 100 ? '...' : ''),
      ...
      try {
        ...
        console.log(
          `[CLIENT-${requestId}] Sending fetch request to /api/substitution...`
        );
        const fetchStartTime = Date.now();
        ...
        const fetchDuration = Date.now() - fetchStartTime;
        console.log(`[CLIENT-${requestId}] Substitution fetch completed:`, {
          status: response.status,
          statusText: response.statusText,
          ok: response.ok,
          fetchDuration: `${fetchDuration}ms`,
        ...
        console.log(`[CLIENT-${requestId}] Substitution response received:`, {
          success: result.success,
          totalDuration: `${totalDuration}ms`,
          responseSize: JSON.stringify(result).length,
          hasError: !!result.error,
        ...
        if (!result.success) {
          console.error(`[CLIENT-${requestId}] Substitution API Error:`, {
            error: result.error,
            details: result.details,
            status: response.status,
          });
        ...
        const geminiResponse = result.data as GeminiResponse;
        console.log(`[CLIENT-${requestId}] Substitution Success:`, {
          textLength: geminiResponse.text?.length || 0,
          textPreview:
            geminiResponse.text?.substring(0, 200) +
            (geminiResponse.text?.length > 200 ? '...' : ''),
        ...
        console.error(
          `[CLIENT-${requestId}] Substitution error after ${totalDuration}ms:`,
          {
            errorType: err?.constructor?.name || 'Unknown',
            errorMessage,
            stack: err instanceof Error ? err.stack : undefined,
        ...
        const finalDuration = Date.now() - startTime;
        console.log(
          `[CLIENT-${requestId}] Substitution request completed in ${finalDuration}ms`
        );
        setLoading(false);
      }
    },
